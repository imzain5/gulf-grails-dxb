/**
 * The catalogue the site ships with.
 *
 * This is the starting inventory — thirty pairs, hand-written, with the studio
 * photography that lives in `public/assets/products`. It is the fallback the
 * storefront renders whenever the live catalogue in Blob storage is missing:
 * before the owner has saved anything from /admin, in local development
 * without a Blob token, and if a read ever fails. That means the site can
 * never render an empty shop.
 *
 * Once the owner saves from /admin the live catalogue takes over completely
 * and this file stops being read for product data. Editing it after that point
 * changes nothing on the live site — edit the catalogue at /admin instead.
 */
...
export const SEED_PRODUCTS: Product[] = RAW.map((r) => {
